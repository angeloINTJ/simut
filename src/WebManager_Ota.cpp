 ...
 * @brief Endpoints OTA — GET /api/backup (download .bkp).
 *
 ...
 * arquivos da LittleFS, então casa com PERM_FILE_READ. */
 ...
 /* Alpha /psz/pcrc no header pro browser verificar download íntegro
...
void WebManager::handleApiRestoreUploadData( ) {
 ...
 if (is_stage) {
 /* Pre-check perm ADMIN-ONLY : OTA stage apaga 1 MB de
...
void WebManager::handleApiRestoreFinish( ) {
 ...
 /* dry-run validate ANTES de remontar LFS — depois do
 ...
 * reboota.
 * demais (default /6 testing) → remonta LFS, sem
 * metadata. */
...
void WebManager::handleApiOtaStagingTest( ) {
 /* OTA staging selftest: DESTRUTIVO (apaga 1 MB) — ADMIN-ONLY . */
 if (getAuthPerms( ) != PERM_FULL_ADMIN) {
 _server.send(403, "text/plain", "Forbidden — admin only");
 return;
 }
...
void WebManager::handleApiOtaApply( ) {
 /* OTA apply: DESTRUTIVO IRREVERSÍVEL — ADMIN-ONLY . */
 if (getAuthPerms( ) != PERM_FULL_ADMIN) {
 _server.send(403, "text/plain", "Forbidden — admin only");
 return;
 }
