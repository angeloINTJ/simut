 ...
 */
 ...
 /* /api/save_net substituido por /api/commit_all */
 _server.on("/api/reset_touch_cal", HTTP_POST, std::bind(&WebManager::handleResetTouchCal, this));
 /* user_add/del/rst substituidos por /api/commit_all */
 ...
 /* OTA: download de backup completo da LittleFS.
 * Alpha /response também inclui X-Backup-PSize/X-Backup-PCrc
 * pro browser verificar integridade antes de aceitar OTA. */
 ...
 /* OTA: rota única para validate/apply (modo no query param ?op=).
 ...
 * (provável buffer interno do WebServer arduino-pico por rota). */
 ...
 /* OTA: endpoint smoke-test do staging POSTERGADO pra 
 ...
 * função real (upload de firmware). Validação da é via reuso
 * dentro do endpoint /api/firmware?op=begin da . */

 /* OTA: dispara apply do update pendente (rota separada de
