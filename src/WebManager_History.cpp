...
#include "ota/backup.h" /* ota::crc32_update pra screenshot_chunk */
#include <LittleFS.h>
#include <time.h>
...
void WebManager::handleApiClearLogs( ) {
 ...
 /* Remove também logs CSV legados ( ) */
...
/* helper compartilhado por handleApiScreenshot + handleApiScreenshotChunk.
...
void WebManager::handleApiScreenshot( ) {
 ...
 /* refactor pra eliminar perda/corrupção de rows.
...
/* screenshot por chunk com CRC32 — integridade verificável.
 *
...
void WebManager::handleApiScreenshotChunk( ) {
 ...
 /* bump handler deadline (mesmo padrão handleApiScreenshot).
 ...
 * /heap-alloc on demand em vez de static BSS — economiza 15 KB
 ...
 * com 503 se heap insuficiente; cliente pode tentar de novo. */
 ...
 /* multi-sample 3x + majority vote via helper
