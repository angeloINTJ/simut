...
public:
 ...
 /** /injeta toque simulado em (x, y). Para um único frame:
 ...
 * (gerar screenshots de todas as telas via API). */
 ...
 /** /força MODE_GRAPH_VIEW pra screenshot automation
 ...
 * sem chave TR. Não segue mudanças de idioma. */
 ...
 void forceDashboard( );
 /* reseta o timer de idle touch (30s → forceDashboard guard).
 ...
 * estável durante a captura via /api/screenshot (~5s). */
 ...
 /** /força repintar do keypad da tela MODE_AUTH no próximo
 ...
 */
 ...
 /* marcador "TFT já foi inicializada alguma vez".
 ...
 * pula a re-inicialização — TFT retém última frame, evita flash branco. */
 ...
 /* flags cooperativas de quiet mode (ver requestQuietMode).
 ...
 * save+reload, saveConfiguration interno chama de novo). */
 ...
 volatile bool _langChanged = false; /**< /setLanguage( ) seta;
 ...
 * blit externo do strip já cobre a região). */
 ...
 /* EXT-005 root cause fix : ponteiros DEVEM ter `= nullptr` explícito.
 ...
 * lixo e o primeiro `_ts->begin( )` ou `_tft->begin( )` travava Core 1. */
 ...
 /* full-screen render reusa `_canvasWide` (320×45)
...
public:
 /* ── F-DISPLAY-ATOMIC : full-screen render via 3 strips de 80px ──
 ...
 * o browser consome sem unaccent). Servido por GET /api/lang. */
 ...
 /** /meta info do .lng ativo (name + code) p/ /api/perms popular
