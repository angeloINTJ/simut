...
void WebManager::handleDelete( ) {
 ...
 /* hot-reload de tema custom em delete (mesmo padrão do upload) —
...
void WebManager::handleApiLs( ) {
 ...
 /* rejeita early em vez de loop O(n²) com replace.
 ...
 * (caso upload em /test ficou invisível pra delete). */
 ...
 /* dirs no root são enumeradas pelo loop principal abaixo
 ...
 * sem entries). Permite mostrar pastas custom no UI. */
 ...
 if (batch[i].isDir) {
 /* dirs emitidas inclusive em "/" — antes pulava no root
...
void WebManager::handleApiMkdir( ) {
 ...
 if (ok) {
 if (!alreadyExisted) {
 /* README.md placeholder garante que pasta vazia persista
 ...
 * de ~1 por chunk HTTP para ~1 a cada 8 KB. */
 ...
 /* hot-reload de tema custom — re-scan se .thm subiu
