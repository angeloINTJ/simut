...
/* timeout variant — caller decide se aborta. */
...
void StorageManager::enterFlashSafeMode( ) {
 /* se estamos dentro de saveConfiguration com quiet mode
...
bool StorageManager::begin( ) {
 /* REMOVIDO enterFlashSafeMode wrap dos mkdirs.
 ...
 * LFS protege seus próprios writes — não envolver. */
 ...
 /* removidos README.md placeholders boot-time pra sysDirs.
 ...
 * ainda cria README.md em pastas custom — esse path é
 * fora de wrap e safe. */

 /* restore da config após OTA apply.
 *
 ...
 *
 * /NÃO envolver em enterFlashSafeMode. Tentativa
 ...
 * salva um PIN != "1234". Setado aqui (loadDefaults = factory reset). */
 ...
 /* inicializa overlay NetworkTimeData via helper (magic
 ...
 * original perdeu-se em algum reboot — não dá pra recuperar. */
 ...
 /* limitamos TOTAL_LANGS de 8 para 2 (EN+PT). Devices que
 ...
 */
 ...
 /* entra em quiet mode cooperativo. Core 1 é sinalizado
...
/* lookup do ambient (DHT22) no calib.csv. Chave = picoUID 16 hex
