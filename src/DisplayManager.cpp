...
#include "HelpLicenseEN.h" /* LICENSE_TEXT_EN inline em PROGMEM */
#include <LittleFS.h>
...
/* reduzido de 8 para 2 idiomas (EN + PT) para
 * economizar flash. Os outros 6 idiomas ficaram no histórico git.
 * CON-002 : TOTAL_LANGS local substituído por LANG_COUNT do enum
...
static void loadLicenseFromFs(int langIdx) {
 ...
 /* EN sempre do PROGMEM (LICENSE_TEXT_EN), sem dependência de FS. */
 size_t i = 0;
 char c;
 ...
 * acesso seguro nesses contextos. */
 ...
 /* força re-render do boot screen pra retraduzir bootLogs já
...
void DisplayManager::pauseRendering(bool pause) {
 ...
 /*
 * U23 ( ): timeout-based lockout em vez de blocking
...
/* durante reboot via web, garantir que o dispatcher do loopCore1
...
/* força tela de gráfico (slot 0). Útil pra automação de
...
void DisplayManager::forceGraphView( ) {
 ...
 /* graph render path requer _repaintGraph (linha 783-784).
...
/* injetar toque simulado pra automação (screenshot capture).
...
/* abordagem HARD-RESET (substitui a cooperativa
...
void DisplayManager::loopCore1( ) {
 ...
 /* alocações de heap preservadas entre resets.
 ...
 * Força delta render na próxima iteração para atualizar dados. */
 ...
 while (true) {
 /* abordagem cooperativa removida (não era
 ...
 * mutex_try_enter falha — evita flicker do overlay. */
 ...
 /* durante boot screen (state.isBooting=true após
...
void DisplayManager::render(const SystemState& state) {
 if (state.isBooting) {
 /* _langChanged força fullRedraw pra retraduzir bootLogs já
 ...
 * suffix. /comparação inclui key, suffix E idioma ativo
 ...
 * pelo Core 0). Fallback é gerado no próprio _licenseBuf se FS missing. */
 ...
 /* dots de página removidos a pedido do user — contador "N/M"
