...
/* BOOT_LOG fan-out — Serial USB CDC + UART1 raw.
...
/* scratch[5] magic — orchestrator seta antes do applier_reboot
...
void AppManager::setup( ) {
 /* UART1 init PRIMEIRA coisa pra ter diagnóstico desde
 ...
 * USB CDC nem PLL_USB, só HW UART). */
 ...
 /* power-cycle CYW43 SEMPRE no setup( ).
 *
 * Antes : só power-cycle se scratch[5] == POST_OTA_APPLY_MAGIC
 ...
 * compat com diagnóstico de POST_OTA_APPLY_MAGIC se alguém setá-lo. */
 ...
 /* ignoreFlowControl(true) — Arduino-Pico SerialUSB::write
 ...
 * não conectado. Aceitável pra debug; revisitar pós-v4 stable. */
 ...
 uart_putc_raw(uart1, '%'); /* pos Serial.begin + ignoreFlowControl */

 delay(1000);
 uart_putc_raw(uart1, '&'); /* pos delay(1000) */
 ...
 */
 ...
 _displayMgr->begin( );
 /* startCore1 DEFERIDO pra DEPOIS de _storageMgr->begin( ).
 ...
 * boot status mensagens até step 6.5 (~10-15s mais tarde). */
 ...
 /* Achado #7 fix — aguardar Core 1 estar PRONTO antes
 ...
 * alpha10 captura HW (boot trava entre step 3 e 4).
 *
 ...
 * não trava). */
 /* step 3.5 (wait Core 1 ready) também deferido. */
 ...
 * (`conf system touch reset` ou recalibrar) ou power cycle limpo. */
 ...
 /* wire quiet mode cooperativo para saveConfiguration.
 ...
 * ops sem cascatas de lockout IRQ stuck. Retorna true só se Core 1 ACKed. */
 ...
 /* AGORA é seguro startar Core 1 — flash ops de mountFS,
 ...
 * vai reformatar LFS aqui antes de subir o web em modo
 * "aguardando restore".
 *
 ...
 * TouchPriority::isActive( ) durante o boot. */
 ...
 /* nome do BT visível na rede agora vem do `cfg.deviceName`
 ...
 * LOG_CODE pra trilha de auditoria. Plaintext nunca persiste em flash. */
 ...
 if (pw && pw[0] != '\0') {
 /* OTP via UART bridge — REMOVIDO em alpha34
 ...
 * Default range 200..3900 cobre a maioria dos painéis XPT2046. */
 ...
 /* SerialBT.begin movido pra DEPOIS de _netMgr->begin (ambas
