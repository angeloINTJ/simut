 ...
 * @brief Acesso de baixo nível à área de staging (OTA).
 *
 ...
 *
 * Sequência:
 * 1. Salva flag "FS in staging mode" (futuro — ).
...
/* variante sem erase upfront — caller faz erase on-demand. */
 ...
 *
 * Em caminho de APPLY ( ), staging_session_end NÃO é chamado —
 * o aplicador continua com LittleFS desmontada e reboota.
 */
bool staging_session_end(StorageManager* storage);

/**
 * @brief Self-test do path raw flash (acceptance criteria).
 *
