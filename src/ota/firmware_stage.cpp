...
/* On-demand erase sector-by-sector avoids the ~13s sync erase in
 * UPLOAD_FILE_START that dropped the TCP socket
 * (client disconnected while WebServer did not return from callback).
 * Each flush_page only erases the sector it will write (~50ms).
 *
 * Statics instead of struct members to avoid recompile cascade via
 * firmware_stage.h (WebManager.h includes it). OTA stage is serial — only one
 * session at a time. */
static uint8_t s_sectors_erased[32]; /* 256 sectors ÷ 8 bits */
...
bool stage_session_begin(StageSession& s, StorageManager* storage) {
 ...
 /* Snapshot BEFORE unmounting LFS (needs LFS-readable). Commit at end. */
...
bool stage_session_end(StageSession& s) {
 ...
 /* Snapshot commit at end — erases sector 255 (last, reserved for
 * snapshot) and writes. Non-fatal: without snapshot, device boots in
 * factory post-apply and user restores via .bkp. */
 if (s_snapshot_len > 0) {
