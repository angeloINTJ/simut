 ...
 * @brief Implementation of the restore state machine (OTA).
 *
...
/* Ensures parent directories of the path exist (for LittleFS.open(..., "w")). */
static bool ensure_parent_dirs(const char* path) {
 /* LittleFS implicitly creates parents in most cases, but we guarantee. */
...
/* ---------------------------------------------------------------------------
 * Walk + commit/cleanup unified.
 *
 * We keep an in-memory list of paths created during the
 * session. Avoids recursive walk of LittleFS (which would duplicate
 * backup.cpp/walk_dir code and pull more lib symbols).
 * ------------------------------------------------------------------------- */

/* Global pool; sized for ~32 files of average path length.
 * If overflow, restoration stops with IO_ERROR — acceptable for v1. */
...
/* Strategy: write directly to final path in APPLY mode.
 * commit is no-op. rollback (on failure) deletes partially written files.
 *
 * Trade-off vs "atomic strategy via .tmp + rename": if CRC
 * fails at the end of upload, final files have already been touched. Mitigation:
 * client MUST call /api/restore/validate before /api/restore/apply to
 * ensure prior integrity. Without this call, integrity failure applies
 * partial damage. Savings: ~16 KB of flash that LittleFS.rename pulled. */
static uint32_t commit_all_tmps( ) {
 uint32_t n = s_tmp_count;
 tmp_list_reset( );
 return n; /* files are already at final path since write */
}
...
static void session_reset(RestoreSession& s) {
 ...
 s.status = BackupStatus::INTERNAL_ERROR; /* replaced when finish() */
 s.header_filled = 0;
...
void restore_session_begin(RestoreSession& s, RestoreMode mode) {
 ...
 /* Clean up any orphan .restore_tmp from previous runs before starting. */
...
/* Parse and validate header (called when header_filled == sizeof(BackupHeader)). */
static void on_header_complete(RestoreSession& s) {
 ...
 /* Empty backup. Expected CRC is 0xFFFFFFFF ^ xor-out = 0. */
...
static void on_path_complete(RestoreSession& s) {
 ...
 if (s.mode == RestoreMode::APPLY) {
 /* Write directly to final path (no rename). See commit_all_tmps.
 *
 * F-RESTORE fix: feed WDT before each operation that can trigger
 * LittleFS GC (mkdir, open with truncate). Under fragmented LFS
 * (>70%), internal GC blocks for seconds. On backup with 32 files
 * the total accumulated time exceeded WDT 8s and the restore lost the tail
 * of the payload (lang/themes/last history). */
 watchdog_update( );
 ...
 /* Tracking: for rollback on CRC mismatch we delete the written ones. */
...
bool restore_session_feed(RestoreSession& s, const uint8_t* data, size_t len) {
 ...
 size_t take = (len < need) ? len : need;
 /* Entry header bytes COUNT toward payload_crc + payload_remaining. */
 ...
 break; /* unreachable, but keep -Wswitch happy */
 }
 }
...
BackupStatus restore_session_finish(RestoreSession& s, bool* fs_modified) {
 ...
 /* If still mid-stream, mark as truncated. */
