 ...
 * @brief Implementation of the .bkp backup generator (OTA).
 *
 ...
 * ---------------------------------------------------------------------------
 *
 * 256-entry table in flash (1 KB const). Tradeoff: ~1 KB of flash in
 * exchange for ~8x more throughput than bit-by-bit calculation. Critical here
 * because backup processes all LittleFS files under the Web WDT.
 */
...
uint32_t encode_version_u32(const char* s) {
 ...
 if (v[idx] > 255u) v[idx] = 255u; /* saturate at 8 bits */
 } else if (c == '.') {
 idx++;
 } else {
 break; /* anything non-numeric/non-dot ends parsing (e.g. "-rc1") */
 }
 }
...
/* ---------------------------------------------------------------------------
 * LittleFS walk (recursive DFS, depth-limited)
 * ------------------------------------------------------------------------- */

static constexpr int MAX_WALK_DEPTH = 8;
static constexpr size_t IO_CHUNK = 512;

/* Visitor callback. Return false aborts the walk (used to propagate I/O error). */
...
static bool walk_dir(const String& dir_path, VisitFn visit, void* ctx, int depth) {
 ...
 while (d.next( )) {
 watchdog_update( ); /* long operation — feed WDT */
...
static bool scan_visitor(const char* full_path, File& f, void* vctx) {
 ...
 /* Updates CRC and payload size exactly as emit will write:
 * [BackupEntry packed (6 B)] [path bytes] [content bytes]
 *
 * BackupEntry is serialized in little-endian via 6 literal bytes. */
 ...
 if (got <= 0) return false; /* I/O error or size changed between stat and read */
...
bool backup_scan(BackupScanResult& out) {
 ...
 out.payload_crc32 = ctx.crc ^ 0xFFFFFFFFu; /* final xor-out */
...
bool backup_emit(Print& out,
 ...
 /* Header CRC = CRC of first 36 bytes (everything except header_crc32). */
