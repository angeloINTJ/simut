 ...
 * @brief Aplicador SRAM-resident — real (modo RAW). VALIDADO.
 *
 * @details ✅ STATUS: Validado em HW ( ). v3.43.9 sector-0-fix +
 ...
 * Fix /substituí por inlines MMIO puros (applier_wdt_feed
 * + applier_reboot). Fix /corrigido bit do TRIGGER de
 ...
 * Fix /programar sector 0 ISOLADO (erase + program 4 KiB)
 ...
 * COMPARTILHA partição com LittleFS. vai integrar
 ...
 *
 * F-OTA-BOOTLOOP fix /alinhar com SDK em vez de 0xFFFFFFFF. */
 ...
 * RAM intacta).
 *
 * external linkage (sem `static`) — também é usado por
...
bool __not_in_flash_func(ota_applier_run)(const UpdateMetadata* meta) {
 ...
 /* Modo RAW ( ): compressed_size == raw firmware size (já alinhado
 ...
 * v3.43.7+/flash_range_program(0,...) silenciosamente NÃO
