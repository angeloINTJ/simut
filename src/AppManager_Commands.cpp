 ...
 * for número bem-formado (evita "abc".toInt( )==0 aplicar tema 0). */
 ...
 /* wrappa save + reload de sensores no mesmo
 ...
 * cai no fallback "lastRecorded - 30 days". */
 ...
 /* CMD_DBG_SENSOR_HISTORY_ALL removido (debug TEST-ONLY). */

 case CMD_GOTO_SCREEN: {
 /* muda tela TFT via show*Screen( ) direto.
 * Bypass handleTouch (gates de pressão). Pra screenshot
 * automation. Strings curtas pra economizar flash.
 * /também reseta _lastTouchTime — sem isso, a guarda
 ...
 * resetTouchIdle( ) dá 30s de janela pra capturar. */
 ...
 case CMD_TOUCH_SIM: {
 /* injeta toque simulado (x, y) screen-space.
