...
/*
 * handleSaveSystem — versao minimal pos-.
 * Versao pre-foi substituida por handleApiCommitAll. Mantida apenas
 ...
 * Usado quando cliente envia int/float sem aspas. */
 ...
 /* parsing estrito de campos numéricos.
 ...
 * rejeita não-numéricos; em caso inválido, preserva valor atual. */
 ...
 if (has("t_key")) {
 /* se valor contém "***" significa que veio do GET mascarado
 * e user não editou — manter cfg.telApiKey atual. Senão sobrescrever. */
 ...
 /* flag NTP enable/disable (overlay NetworkTimeData). */
 ...
 * {"type":"reset","id":5}]}} */
 ...
 int objStart = 0;
 int safety = 0; /* cap iterações (users actions: max 8) */
 ...
 /* dns_auto + dns2 (primário manual reusa cfg.staticDns).
 ...
 * no modo manual com DHCP=true (sem os outros campos staticIp/mask/gw). */
 ...
 /* mostra mensagem no display ANTES de qualquer flash I/O.
...
/* handleSaveNetwork removido em substituido por handleApiCommitAll. */
