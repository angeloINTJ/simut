...
void SensorManager::initRuntimeSensors(const SystemConfig &cfg) {
 ...
 /* usa cfg.ambientSensor.hwId/friendlyName em vez de hardcoded
...
void SensorManager::handleSensorResult(RuntimeSensor &s, bool success, float v1, float v2, const char* errorMsg) {
 ...
 if (!s.inErrorState) {
 /* ambient (DHT22) aplica offset em ambas as grandezas;
...
/* aplica os offsets do ambient (DHT22) em runtime. ID/nome são
