 ...
 * aborta handler limpa em vez de deixar o WDT disparar. */
 ...
 /* broken-pipe observability. Antes safeSend retornava false
 ...
 * 1×/5s (throttle anti-spam quando handler envia muitos chunks). */
 ...
 // ReadGuard movido para StorageManager.h ( ) — agora público.
 ...
 void handleSaveSystem( ); /**< minimal — so theme-switch do dashboard */
 void handleApiCommitAll( ); /**< save-all + reboot */
 /* handleSaveNetwork substituido por handleApiCommitAll */
 ...
 /* handleApiUserAdd/Del/Reset substituidos por handleApiCommitAll */

 void handleNotFound( );
 void handleLangJs( );
 void handleStyleCss( ); /**< /F-WEB-DEDUP — CSS comum cacheável */
 ...
 void handleApiScreenshotChunk( ); /**< /chunked com CRC32 */
 ...
 /* POST /api/set_time — aplica manual RTC via
 ...
 * pois o user espera ver a hora atualizada na mesma resposta. */
 ...
 /* integrado no /dashboard. 2 endpoints.
 ...
 * calib.csv com VERSION=epoch (NTP-gated) */
 ...
 /* OTA: backup completo da LittleFS atrelado ao chip_id (.bkp).
 * Implementação em WebManager_Ota.cpp; formato em src/ota/backup_format.h. */
 void handleApiBackup( );

 /* OTA: validação + restore de .bkp. Único handler para ambos os
 ...
 * duplicação de std::function/std::bind no .text. */
 ...
 /* OTA: handler stub pra staging_test — postergado pra 
 * (ver WebManager_Core.cpp). Mantido como declaração futura. */
 void handleApiOtaStagingTest( );

 /* OTA: dispara apply do update pendente. Em 7a (no-op), aceita
 ...
 * restore corrompe mid-write). */
 ...
 /* OTA: upload do firmware .bin.gz pra staging via
 ...
 * cobre o caso patológico de 2 sessões competindo por LittleFS. */
 ...
 /* OTA: adapter que expõe safeSend para o ota::backup_emit (Print&). */
