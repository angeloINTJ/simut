...
/* Melodies for SND_ATTENTION (confirmation/attention screens). */
...
const MelodyDef SoundManager::MELODIES[5][SND_MELODY_VARIANTS] = {
 ...
 /* SND_ATTENTION — notification sounds for confirmation screens. */
 {
...
void SoundManager::loadConfig(const SoundConfigData* data) {
 /* Magic byte: 0xAB = legacy (v3.32.2-, without Attention field),
 * 0xAC = current (v3.32.3+, with Attention).
 * Other values → full defaults (corrupted config or first boot). */
 ...
 if (data->magic == 0xAB) {
 /* Migration v3.32.2- → Attention fields do not exist in old
 * config. Default = ON with first melody to preserve UX (confirmation
 * screens play sound immediately without user needing to enable). */
 _enableAttention = true;
 _melAttention = 0;
 } else {
