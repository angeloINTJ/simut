...
/* melodias para SND_ATTENTION (telas de confirmação/atenção). */
...
const MelodyDef SoundManager::MELODIES[5][SND_MELODY_VARIANTS] = {
 ...
 /* SND_ATTENTION — toques de notificação para telas de confirmação. */
 {
 ...
 * Outros valores → defaults completos (config corrompida ou primeira boot). */
 ...
 if (data->magic == 0xAB) {
 /* Migração v3.32.2- → /campos Attention não existem na config
...
void SoundManager::fillConfig(SoundConfigData* data) const {
 ...
 /* schema ganhou bit ATTENTION (flags) + 3 bits melAttention
