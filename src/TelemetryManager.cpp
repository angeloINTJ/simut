 ...
 */
 ...
 /* intervalo efetivo dinâmico inline.
 ...
 */
 ...
 /* aborta se heap está criticamente baixa.
 ...
 * behavior (já reproduzido em stress test 4 reboots em
 ...
 *
 * F-MEM-NOCACHE : heap permanece estável em ~50 KB sem
 ...
 */
/* lógica inline no update( ). Sem accessor exposto
...
uint8_t TelemetryManager::safeBatchLimit(uint8_t configured) {
 ...
 /*
 * HEAP_RESERVE diferenciado por encryption :
 ...
 */
 /* HEAP_RESERVE HTTPS aumentado 24K → 32K. Stress test 
...
bool TelemetryManager::collectBatch(std::vector<BinaryHistoryRecord>& batch, uint32_t& newCursor) {
 ...
 /* detecção cursor-no-futuro. Se lastCursor > now + 1 dia,
 ...
 * usar último timestamp gravado - 30 dias para limitar varredura. */
 ...
 /* codec V2 (delta + anchor). Substitui o read
 ...
 * e causava bootloop em telemetrias grandes pós-boot (validado HW ).
 ...
 */
 ...
 /* ID customizado via calib.csv (linha `t<id>`) sobrescreve
...
void TelemetryManager::refreshPendingCount( ) {
 ...
 /* mesmo motivo do collectBatch — lia 28 B raw
