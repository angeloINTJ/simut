...
#include "StorageManager.h" /* getBoardSerialNumber em System Status */
...
void DisplayManager::showMuteConfirm( ) {
 ...
 _repaintSettings = true;
 mutex_exit(&_stateMutex);
 /* dispara SND_ATTENTION ao entrar na tela de confirmação. */
...
void DisplayManager::drawSettingsSounds( ) {
 ...
 /* 9 items (acrescentado Attention entre Web e Mute). 3 páginas. */
 ...
 /* ordem reorganizada — volumes primeiro, mute por último.
...
void DisplayManager::drawMelodySelect( ) {
 ...
 /* typeIdx==4 (Attention) usa label hardcoded condicional. */
 ...
 */
 ...
 /* serial do Pico — chave do calib.csv pra ambient (DHT22). */
 ...
 * /valores longos (SSID até 32 chars, MAC 17 chars) que não cabem
