 ...
 * @brief Implementacao do codec v2 (delta + sensor-mask + anchor).
 */
...
size_t readVarintZ(const uint8_t* buf, size_t bufLen, int32_t& out) {
 ...
 return 0; /* truncado ou varint invalido (>5 bytes) */
}
...
size_t historyEncodeRecord(const BinaryHistoryRecord& rec,
 ...
 s.initialized = true;
 s.recordsSinceAnchor = 1; /* anchor conta como record 0; proximo delta sera o 1 */
 ...
 * Bits 12..15 reservados = 0. */
 ...
 p[1] = (uint8_t)((mask >> 8) & 0x0F); /* garante top 4 bits = 0 */
 p += 2;

 /* Δepoch sempre presente. Em uso normal = intervalo configurado (1B). */
 ...
 /* Atualiza state apenas para campos efetivamente presentes. */
