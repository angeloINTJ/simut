 ...
 * Used in login verification with hashVersion >= 1. */
 ...
 /** Slot of the active account holding this digest, or -1. */
 int pinDigestOwner(const uint8_t* digest, int exceptSlot = -1) const;
 /** Set (or with "" clear) an account's PIN in RAM. false when the PIN is
  *  malformed or already identifies another account — PINs must be unique
  *  because the panel identifies BY them; `conflict` says which one. Does not
  *  save: callers batch this with whatever else they change. */
 bool setUserPin(int slot, const char* pin, int* conflict = nullptr);
 static bool userHasPin(const UserAccount& u) {
 ...
 * time; the sole caller is the serial announcement in AppManager_Boot. */
 ...
 /** Schema version the loaded config was migrated FROM (20..23), or 0 when it
  *  was already current. Clears on read, for the same reason as above: the
  *  migration ran before the logger existed, and it is worth one line. */
 uint16_t takeMigratedFromVersion( ) {
  uint16_t v = _migratedFromVersion;
  _migratedFromVersion = 0;
  return v;
 }
 ...
 * change (boot or day rollover). */
 ...
 /** Factory line template of the alarm line (v24 shape). */
 static const char* const ALARM_LINE_TEMPLATE_DEFAULT;
 /** v24: a stored template that still equals an older default follows the
  * default forward; an edited one is the operator's and is kept. */
 static void upgradeDefaultAlarmTemplate(AlarmTelConfig& a);
 /** v24: one reader for every legacy schema (v20, v21/v22, v23). Reads the
  * whole file into a transient heap buffer, checks the CRC over the raw bytes
  * and lets configMigrateLegacy( ) place the segments; the per-version
  * defaults and the v24 finish (PIN salt, admin PIN) follow. `kind` is a
  * CfgLegacyKind — typed as uint8_t so this header need not include
  * ConfigMigrate.h. */
 bool loadMigrateLegacyBlob(File& f, size_t fileSize, uint8_t kind, SystemConfig& outCfg);
 /** v24 finish for a migrated blob: generate the PIN salt, and turn the old
  * plaintext panel PIN into the admin's digest — or "1234" with the forced
  * change armed, when the old PIN had characters the numeric keypad cannot
  * type. Zeroes the plaintext. */
 void finishMigrationV24(SystemConfig& cfg);
 static void setMustChangePinIn(SystemConfig& cfg);
