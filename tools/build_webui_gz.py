...
OUTPUT_FS_HEADER = os.path.join(PROJECT_DIR, "WebUI_FS.h")
OUTPUT_FS_DIR = os.path.join(PROJECT_DIR, "data", "web")
...
def generate() -> None:
    ...
    # Header alternativo apenas com mapping nome → path em LittleFS.
    # Usado quando WEBUI_FROM_FS=1 build flag (libera ~74 KiB de flash).
    fs_lines = [
        "// Auto-generated by tools/build_webui_gz.py — do not edit.",
        f"// Source hash: {input_hash}",
        "//",
    ...
    # Garante diretório de saída para os .gz na LittleFS image.
    os.makedirs(OUTPUT_FS_DIR, exist_ok=True)
    ...
    for name, html_content in matches:
        ...
        # Também emit como arquivo gz na LittleFS image (data/web/<name>.gz).
        # Path em runtime: /web/<name>.gz
        fs_filename = f"{name.lower()}.gz"
        fs_path     = os.path.join(OUTPUT_FS_DIR, fs_filename)
        with open(fs_path, "wb") as fp:
            fp.write(compressed)
        fs_files_emitted.append((name, fs_filename, length))
        fs_lines.append(
            f'constexpr const char* {name}_PATH = "/web/{fs_filename}";'
        )

    out_lines.append("}")
    fs_lines.append("")
    fs_lines.append("} /* namespace WebUI_FS */")

    with open(OUTPUT_FILE, "w", encoding="utf-8") as f:
        f.write("\n".join(out_lines) + "\n")
    with open(OUTPUT_FS_HEADER, "w", encoding="utf-8") as f:
        f.write("\n".join(fs_lines) + "\n")
    ...
    print(
        f"build_webui_gz: emitted {len(fs_files_emitted)} files to {OUTPUT_FS_DIR}/ "
        f"+ WebUI_FS.h"
    )
