...
# ── Portão: um helper usado numa página tem de existir NELA ────────────────
# Cada página do WebUI é um blob gz independente e um <script> só enxerga o
# que está na própria página mais o que /lang.js trouxer. Em 2026-09-20 um
# window.hasPanelPin( ) foi definido ao lado do dicionário de eventos
# (HIST_PAGE) e usado na CFG_PAGE: o TypeError derrubou o preenchimento
# inteiro do formulário e a página de configuração parou de carregar. Nada
# falhou na compilação, nada falhou na geração, e o defeito só aparecia ao
# abrir aquela página.
#
# `window.foo(...)` é a forma que o projeto usa para helper compartilhado, e é
# exatamente a que atravessa páginas sem avisar. As APIs do próprio navegador
# ficam de fora da conta.
_BROWSER_GLOBALS = {
    "matchMedia", "addEventListener", "removeEventListener", "setTimeout",
    "setInterval", "clearTimeout", "clearInterval", "requestAnimationFrame",
    "getComputedStyle", "fetch", "alert", "confirm", "prompt", "open",
    "scrollTo", "print", "postMessage", "btoa", "atob", "isNaN", "parseInt",
    "parseFloat", "encodeURIComponent", "decodeURIComponent", "Blob", "URL",
}


def _check_page_globals(matches) -> None:
    by_name = {name: body for name, body in matches}
    lang = by_name.get("LANG_JS", "")
    lang_defs = set(re.findall(r"window\.(\w+)\s*=", lang))
    problems = []
    for name, body in matches:
        if name == "LANG_JS":
            continue
        avail = set(re.findall(r"window\.(\w+)\s*=", body))
        if 'src="/lang.js' in body:   # o gerador acrescenta ?v=<hash>
            avail |= lang_defs
        used = set(re.findall(r"window\.(\w+)\s*\(", body))
        missing = sorted(u for u in used if u not in avail and u not in _BROWSER_GLOBALS)
        if missing:
            problems.append((name, missing))
    if problems:
        lines = [
            "build_webui_gz: helper de pagina usado onde NAO existe.",
            "Cada pagina e um blob gz separado: um <script> so enxerga a propria",
            "pagina e o que /lang.js trouxer. Defina o helper no LANG_JS (que as",
            "nove paginas carregam) ou na propria pagina.",
        ]
        for name, missing in problems:
            lines.append(f"  {name}: {', '.join('window.' + m + '( )' for m in missing)}")
        raise SystemExit("\n".join(lines))


# ── Portão: um token de cor usado tem de estar definido ────────────────────
# `var(--nao-existe)` não é erro de sintaxe: o navegador DESCARTA a declaração
# inteira e segue. Em 2026-09-20 `--fundo-sutil` e `--tinta-fraca` foram
# inventados (os nomes certos são `--superficie-2` e `--tinta-2`) e a busca de
# redes foi ao PR com o botão sem fundo e o dBm sem cor secundária — nos dois
# temas. Nada falhou: nem a compilação, nem o minificador, nem o portão de
# helper. Só apareceu ao ABRIR a página e conferir token por token.
#
# A lista de definidos sai de qualquer `--nome:` em todo o WebUI.h, porque as
# paletas clara e escura são dois blocos `:root` e um token só serve se
# estiver nos dois — o que esta contagem também mostra.


def _check_css_tokens(content: str, matches) -> None:
    # Comentários fora: a primeira versão deste portão acusou `--espaco-N`, que
    # existe só numa frase explicando por que a grade NÃO usa var( ). Um portão
    # que grita por prosa é um portão que se aprende a ignorar.
    strip = lambda t: re.sub(r"/\*.*?\*/", " ", t, flags=re.S)
    defined = set(re.findall(r"(--[a-z0-9-]+)\s*:", strip(content)))
    problems = []
    for name, body in matches:
        used = set(re.findall(r"var\(\s*(--[a-z0-9-]+)", strip(body)))
        missing = sorted(u for u in used if u not in defined)
        if missing:
            problems.append((name, missing))
    if problems:
        lines = [
            "build_webui_gz: token de cor usado sem estar definido.",
            "`var(--x)` sem `--x:` faz o navegador descartar a declaracao inteira",
            "em silencio. Use um token que exista nas DUAS paletas :root.",
        ]
        for name, missing in problems:
