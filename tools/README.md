...
| `measure_savings.py` | Quanto de flash e RAM cada recurso devolve quando desligado: constrói o perfil-base e a variante com só aquele recurso off (derivada de `features.toml`, então `off_exclude` tira mesmo o `.cpp`) e mede a diferença. `--update` grava o piso em `tools/feature_savings.json`; `--check` reprova se a economia caiu (recurso vazou para o sempre-ligado) — ratchet ao contrário do orçamento. Caro (~13 imagens): roda em `savings.yml` (noturno), não no job `gates`. Sabe reprovar: controles +/− conferidos em 26/09. Achados em `docs/analysis/ECONOMIA_DE_RECURSOS.md`. | 2026-09-26 |
...
| `gen_logo.py` | Generates the brand in the Ângulo standard — the disc, the wordmark light and dark, the name alone, the two Powered-by badges, the site's social card and `data/favicon.ico` (731 B, flash byte for byte) — reading every colour from `docs/assets/angulo.css`. Needs fonttools, brotli and pillow; not part of the build. | 2026-09-24 |
...
| `ota_test.py` | Drives one OTA on the bench: login, stage (`/api/restore?op=stage&commit=1`), apply. Does NOT verify the version — the caller reads it back, because the only proof of an installed update is the new version reporting itself. Warning: Stage the image on an alternate port: the router kills long port-80 flows, and any stage borrows the whole LittleFS region. | 2026-09-11 |
