...
| `panel_study_capture.py` | Bench companion of the Ângulo panel study (env `pico_w_uistudy`): switches theme and dashboard layout over the serial CLI, captures every arm with `/api/screenshot`, taps the top card into min/max with `/api/touch`, and checks each layout capture against the tokens the firmware wrote (RGB565 expanded, never another capture) and the 4-px safe area. RAM-only: never sends `write memory`. | 2026-09-25 |
