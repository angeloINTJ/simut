...
[ ] Captura: `boot_screen.bmp` — exibida durante setup() com status keys (TR_BOOT_MOUNT_FS, TR_BOOT_START_LOG, TR_BOOT_INIT_NETWORK, TR_BOOT_INIT_WEB, TR_BOOT_HOLD_AP).

### 4.2 Tela Auth (PIN entrance)

[ ] Captura: `auth_screen.bmp` — keypad 4 dígitos para destravar acesso.

`showAuthScreen(String expectedPin)` em DisplayManager.h:200.

### 4.3 Dashboard principal

[ ] Captura: `dashboard.bmp` — exibe:
...
[ ] Captura: `graph.bmp` — gráfico time-series do sensor selecionado.
...
[ ] Captura: `alarm_action.bmp` — `showAlarmAction(slotIdx)` quando setpoint violado. Botão MUTE para parar buzzer.
...
[ ] Captura: `web_login.png` — username + password + nonce (anti-replay).
...
[ ] Captura: `web_force_chpass.png` — Trocar senha admin do default OTP.

### 5.3 `/` (Dashboard)

[ ] Captura: `web_dashboard.png` — exibe:
- Painel ambiente
...
[ ] Captura: `web_history.png` — gráfico multi-sensor.

API: `/api/history_multi`, `/api/history_days`, `/api/export/history.bin` (CSV-equivalente).

### 5.5 `/alarms` (Configuração de Alarmes)

[ ] Captura: `web_alarms.png` — limites min/max por slot.

API: `/api/alarms`.

### 5.6 `/config` (Configuração geral)

[ ] Captura: `web_config.png` — nome, fuso, NTP, telemetria HTTP, MQTT, history interval.

API: `/api/config` GET, `/api/save_sys` POST.

### 5.7 `/network` (WiFi config)

[ ] Captura: `web_network.png` — SSID, pass, DNS, IP estático/DHCP.

API: `/api/network`.

### 5.8 `/users` (Gestão de usuários)

[ ] Captura: `web_users.png` — até 5 usuários com permissions bits.

API: `/api/users`.

### 5.9 `/files` (File manager + OTA Firmware)

[ ] Captura: `web_files.png` — lista LittleFS, upload/download/delete.
...
[ ] Captura: `web_license.png` — info de licença e sec status.
