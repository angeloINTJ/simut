...
| [AUTHORIZATION.md](AUTHORIZATION.md) | Living | The authorization matrix (`check_authz.py --list` counts the routes; 62 after `GET /api/logcodes`). Gated by `tools/check_authz.py`. |
...
| [analysis/ECONOMIA_DE_RECURSOS.md](analysis/ECONOMIA_DE_RECURSOS.md) | **Living** | Quanto de flash e RAM cada chave devolve quando desligada, medido pelos seis ambientes com `tools/measure_savings.py`; o piso em `tools/feature_savings.json` que o portão impede de regredir, e os achados (bluetooth = 140 kB; o buzzer preso ao SIMUT_AIR) (pt-BR). |
...
| security audit (externa) | 2026-08-29 | [IMPLEMENTACAO](security-audit/IMPLEMENTACAO.md) — ACH-01..08, como cada achado foi fechado. Atenção: ACH-05 e ACH-08 constam como "adiado"/"sem ação" e foram feitos depois, na linha de setembro. |
