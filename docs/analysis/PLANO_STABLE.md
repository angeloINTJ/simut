...
**Este documento é Living.** Cada linha da matriz vira **feito** com a data e o
número, ou continua aberta. Nenhuma vira **feito** por argumento.
...
| Alarmes 2ª linha (HTTP/MQTT) | `alarm_hw_test.py`, `alarm_mqtt_test.py` — atenção: em 21/09 a suíte deu **11/14**, e as 3 falhas são o B12 (corte de 63 chars da CLI), não a linha | 23/08 |
...
| B6 | `pico_w_test_https` a **3.204 B** do teto de OTA (medido 21/09; o 3.308 anterior era de build anterior) | não | Ambiente de bancada, não é imagem de produto. Vira bloqueio se alguém precisar dele no campo |
| B7 | Issue #118 — mDNS do Air | não | Backlog |
| B10 | **Cursor de telemetria pula registro em bloco fora de ordem** — medido em 21/09: **6 de 75.778** registros (0,0079%) em 55 arquivos de dia | não | Fere o item 2 *em silêncio*, e é por isso que não é ruído. Mas o registro **não se perde**: está na flash e sai pelo `/download` e pelo CSV — só a telemetria não o leva. A causa está escrita no firmware (`src/TelemetryManager.cpp:286`): um cursor escalar em tempo não alcança um registro atrás da marca-d'água. Fechá-lo é mudança de formato (cursor vira posição de varredura), não de rótulo. Vira bloqueio se o aparelho continuar **sem dizer** que pulou |
...
O que cada linha tem de produzir para virar **feito**: um **número** ou um **log**,
...
| T2 | Caça ao B1 — **feito em 21/09, 12 corridas, 4,4 h** | `panel_fulltable_test.py` em repetição, com o log binário preservado entre corridas | 3–4 h | ou reproduz com `ctx` e trace utilizáveis, ou N corridas limpas dão a taxa — **deu o segundo: 0 reboots, 328 ciclos de pausa de flash** |
| T3 | Soak — **feito em 21/09, 7,9 h** | `telemetry_bench/soak_a6.py`, ≥ 8 h, coletor vivo e coletor morto, **na `pico_w_release`** | 8 h+ | 0 reboots sem causa; deriva de heap medida e declarada — **feito: 0 reboots, heap −34 B em 7,9 h** |
| T4 | OTA nesta imagem — **feito em 21/09** | ciclo de stage+apply, ida e volta entre v2.6.0-beta e v2.6.1-beta, pela `:8080` | ~1 h | 3/3 nos dois sentidos, `/history` íntegro pelo `fsguard` — **feito: 6/6 e 75.831/75.831** |
| T5 | Queda de rede — **feito em 21/09, 4/4, na `pico_w_release`** | `wifi_outage_test.py` — hoje ele é do **Air**: segura o aparelho em M0 (`air idle 3600` + linha CHARGER da mão) e aponta o Wi-Fi por `system ssid`/`system pass` pela CLI. Num TFT não há M0; na `pico_w_release` a CLI é o console de emergência, que **tem** `system ssid/pass` mas não o resto | ~40 min + conserto | as 3 fases, com o log do aparelho como prova |
...
**T2 e T3 não rodam em paralelo** — o B11 impede. O T3 exige a
...
Atenção: `prerelease=true` **nunca** vira Latest — é o erro que já custou uma tag
neste repositório.

**PROMOVIDA EM 22/09/2026: `v2.7.0` é a Latest.** O mantenedor decidiu
promover com o B1 registrado como conhecido-não-reproduzido-e-instrumentado
(§5.1, saída 1) e com a metade ociosa do B13 documentada com taxa e mecanismo.
O texto abaixo fica como o portão que foi aplicado.

A promoção acontecia quando **B1, V-09 e B13 estivessem fechados** — o portão que
...
| **B13** | **metade**: a de carga fechou e está medida (270 requisições de corpo grande, 0 cortadas, `cgx` = 0, contra 3 e +49 horas antes). A **ociosa** disparou de novo às 08:56 do T3, num `/api/status` de 5 em 5 min, **sem mover contador nenhum** — a mesma assinatura de sempre |
...
**E há uma conta a pagar depois deles:** o B2 e o B3 foram medidos *na imagem
...
| 21/09 | T7 partes A e B: relógio/histórico e leitor de gráficos | feito (parte C bloqueada, ver B8) |
...
| 22/09 | **Retratação:** eu havia escrito que "nenhum dos quatro contadores registra" | Verdade só para `/api/status` (corpo pequeno). Com corpo grande o `cgx` registra, e muito. A frase anterior media um caso e falava de todos |
...
| 22/09 02:0x | **O teste de colisão pegou um defeito da minha primeira versão** | Minutos em `2000+` saturando em 20000 fazia um aparelho de pé há 1000 min gravar `ctx=3000` — que se lê como "heap 0 KB". Lição: uma faixa nova se confere pelo **alcance inteiro, saturação incluída**, não por uma amostra |
| 22/09 02:1x | **3 legendas curtas custaram 4.096 B numa imagem** | O alpha estava contra uma borda de 4 KiB: as três `F("…")` cresceram `.text` em 192 B e `.rodata` em **uma página inteira**, e o portão de orçamento reprovou por 1.288 B. Sem as legendas (que só repetiriam a frase impressa uma linha acima), `.rodata` volta ao lugar. **Δ de 15× entre imagens pela mesma mudança = degrau de alinhamento, não custo de código** — confira `arm-none-eabi-size -A` antes de acreditar no `used` |
...
| 22/09 | **Efeito colateral que não se desfaz** | Apertar a política marca **toda** conta com PIN (`pinAuth.pinMustChange`). Relaxar marca ninguém **mas não limpa**: o bit só cai quando aquele slot tem um PIN **definido** (`setUserPin`), e `/api/config` **não expõe** o bitmap — não dá para ler nem restaurar pela web. As 6 contas com PIN deste aparelho estão marcadas desde o restauro de 21/09 que o mantenedor aprovou |
| 22/09 06:2x | **B13 FECHADO — conferido com o mesmo instrumento, mesmo caminho** | O mantenedor resolveu o caminho de rede e eu medi em vez de acreditar: `b13_probe.py` pelo roteador, porta 80, **270 requisições e 2.145 kB em 53 s — 0 cortadas (0,0%) e `cgx` = 0**. O mesmo laço, no mesmo aparelho e pelo mesmo caminho, dera **3 cortadas e `cgx` +49** às 00:4x. É o A/B que faltava com a variável mudada de propósito por quem podia mudá-la |
| 22/09 06:05 | **T3 recomeçado na imagem que a versão vai publicar** | `pico_w_release` construída de `main` + PR #149 + PR #151 (merge `54b8875`, `.bin` 1.024.948 B, sha256 `7fc22c1782001ff4…`, folga de OTA 15.436 B). `fsguard backup` antes: 59 arquivos, 0 falhas; a config atravessou a gravação inteira (telemetria no coletor real, política de PIN 4/8/3/0) e o `fs_u` ficou em 905.216 B. Linha de base: `heap_lb` 32.324, `fx`=0, `c1kl`/`c1kh`=0. **Escolha do mantenedor: as duas correções na mesma imagem** |
| 22/09 06:05 | **Este soak roda com o coletor MORTO** | O coletor real (192.168.3.206:8080) não responde. É o caso mais duro — exercita fila e retry a corrida inteira — mas **não prova o dreno**, que ficou provado em 21/09 (239 registros numa rodada, 0 faltando) e cujo código não mudou. Se o servidor voltar durante as 8 h, o dreno se prova de novo |
| 22/09 09:08→09:10 | **Os quatro PRs mergeados na `main`** | #149 (V-09), #150 (arnês), #151 (autópsia persistida) e #148 (plano + manual do servidor). Os três de código entraram CLEAN com 10/10; o #148 esperou o `static-analysis` |
| 22/09 09:1x | **A imagem no soak é a da `main`, byte a byte** | `sha256 7fc22c1782001ff4…`, 1.024.948 B, conferido **duas vezes**: depois dos três merges de código e de novo depois do #148 (que tocou `src/WebManager_Commit.cpp`, mas só num comentário). É o que faz a preview valer como medição da imagem final |
| 22/09 06:11 | **O soak foi REINICIADO, e por um motivo que vale a regra** | O #148 trazia também o conserto do `judge( )` do `soak_a6.py` — a versão antiga conta `c1kq` (pausa de flash) como morte do Core 1 e teria enchido a corrida de anomalias falsas. A corrida lançada da preview usava a ferramenta velha. Com 10 min de vida, reiniciar custou nada. **Regra: a ferramenta de análise vem do mesmo commit que a imagem sob teste** |
| 22/09 08:56 | **Retifico o fechamento do B13: a metade ociosa voltou** | Dentro do T3, uma amostra de `/api/status` de 5 em 5 min veio `InvalidChunkLength`. `cgx`, `cgd` e `cgg` ficaram em **0** — nenhum contador mexeu, que é exatamente a assinatura da metade que nunca teve explicação. A de **carga** continua fechada e medida (0 de 270 com corpo grande, às 06:2x). Eu havia marcado o B13 inteiro como fechado; era só metade, e o documento agora diz isso |
| 22/09 09:20 | **T3 em 3h09 de 8h15: 0 reboots, heap −32 B** | 37 amostras com leitura, uptime monotônico, `fx`/`c1kl`/`c1kh` em 0, `heap_lb` 32.313 → 32.281 (mínimo 32.271). `c1kq` 0→1 e `c1n` 1→2 vieram de um `commit_all` meu, e o `judge( )` corrigido os classificou como **nota**, não como morte do Core 1 — a razão de ter reiniciado o soak se pagou aqui |
| 22/09 14:26 | **T3 COMPLETO na imagem final: 8,18 h, 0 reboots** | 99 amostras (92 leram), uptime monotônico 446 → 29.896 s, `heap_lb` 32.313 → 32.271 com piso 32.266 — **−42 B em 8,18 h**, ou −5,1 B/h, mesma ordem dos −34 B das 7,9 h da imagem anterior. `fx`, `c1kl` e `c1kh` em **0** a corrida inteira; batimento do Core 1 máx. **44 ms**. Telemetria contra coletor morto do início ao fim: 49 enviados, 104 falhas, 435 na fila — a fila cresce como projetado. **Refecha o B2 na imagem que a versão publica** |
| 22/09 14:38 | **T4 COMPLETO na imagem final: 6/6 applies** | Três idas e voltas entre a v2.6.0-beta publicada e a build da `main` (`b5bf0cb`), cada apply julgado pela versão que o aparelho relê. Estágio 34,9–35,9 s, CRC conferido nos seis (`dcrc` = `crc32`), volta em 52–56 s. **A imagem sob teste é a build da main, não o ativo publicado da v2.6.1-beta**: as duas dizem "2.6.1-beta" e a versão não as separaria, e o ativo antigo não carrega as correções desta versão |
| 22/09 15:21 | **PROMOVIDA: `v2.7.0` publicada como Latest** | Bump pelo PR #152 (10/10 no CI), tag cortada da `main` `994f058` com `--latest` e `prerelease=false`. **A única mudança em `src/` é a linha da versão** — `git diff` de um hunk só — então a imagem publicada difere da que rodou o T3 e o T4 por **16 bytes de string mais curta e nada mais**. `CONFIG_VERSION` fica em 25: bump de rótulo, não de schema, então ninguém migra config ao atualizar |
| 22/09 15:27 | **Os ativos publicados são byte-idênticos à build local** | `release` 1.024.932 B, `air` 1.035.308 B, `alpha` 990.348 B — `cmp` limpo nos três contra `.pio/build/*/firmware.bin`. O que o CI publicou é exatamente o que foi medido |
| 22/09 15:33 | **O ferro roda a v2.7.0 publicada**, instalada pela própria OTA | Estágio 35,6 s, CRC `F2B2E9C1` batendo, volta limpa: `fx`/`c1kl`/`c1kh` em 0, 5 sensores, config idêntica nos campos conferidos |
| 22/09 15:27 | **O B13 quebrou um backup, e eu deixei passar** | O `fsguard backup` de antes desta OTA morreu no meio com `IncompleteRead` — pegou 8 de 57 arquivos e nem escreveu o manifesto. E eu **encadeei com `&&` depois de um `tail`**, então o status que o `&&` viu foi o do `tail` (zero) e a gravação seguiu sobre um backup incompleto. O backup completo das 14:28 salvou: restaurado, **0 registros faltando** de 76.588. Perdeu-se só a janela 14:28→15:27, ~47 registros de 76.612 (0,06%). **Regra: `cmd \| tail` devolve o status do `tail` — use `set -o pipefail` ou teste o código do próprio comando** |
| 22/09 15:30 | **E o B13 também atrapalha a RESTAURAÇÃO** | Duas entradas voltaram `FAIL` com `IncompleteRead` — mas na leitura de **verificação**, não na gravação: a conferência por registros deu 57 arquivos e **0 faltando**. Ainda assim, é a primeira vez na campanha que ele causa risco real a dado, e não só a uma amostra de teste |
| 22/09 14:38 | **E o estágio de 1 MB passou pela porta 80** | Os seis estágios atravessaram a `:80` sem uma queda, exatamente o fluxo que o roteador matava antes (>12–15 s). É evidência extra, colhida de graça, de que o caminho de rede mudou — a mesma variável que fechou a metade de carga do B13 |
| 22/09 14:4x | **Histórico e config através de 6 OTAs: nada perdido** | O apply zerou o LittleFS como sempre (`fs_u` 61.440). Depois do `fsguard restore`: **57 arquivos, 0 registros faltando** (76.588 no backup, 76.612 no aparelho — os 24 a mais são os não-selados do `.wip` que a restauração absorveu). A config veio **idêntica** nos 11 campos conferidos e as 6 contas estão lá. **Refecha a metade exercitável do B3** |
| 22/09 | **A metade ociosa do B13 tem taxa medida e agora TRÊS sintomas** | 7 de 99 amostras (**7,1%**) não leram, e `cgx` foi de 0 a 4. Os sintomas: `InvalidChunkLength` com só o terminador (3×); `InvalidChunkLength` trazendo **o fim de um corpo JSON** no lugar do cabeçalho (2×, `b'al":19.80}\r\n'`); e um **JSON cortado no meio** sem erro de conexão (1×). TCP não reordena, então os três dizem a mesma coisa: **o corte é do emissor**, no meio do chunk. O código nomeia os dois ramos — `WebManager_Send.cpp:89` conta `cgx` e faz `stop(1)` depois de 4 s sem espaço na janela; `isClientGone( )` cai em `dropAbortedStream( )` **sem** contar |
| 22/09 13:05 | **Gastei uma hora num A/B que não podia decidir — dois defeitos meus** | Testei conexão reaproveitada contra conexão nova, 12 rodadas de 5 min: 0 quebras nos dois braços. (1) **Subdimensionado**: a 4% por requisição, 12 tentativas esperam 0,5 evento — precisaria de ~56 por braço, quase 5 h. É a MESMA armadilha que anotei em 21/09 e repeti. (2) **Alvo errado**: escolhi `/api/login_init`, que responde `Content-Length: 71` e **não é chunked** — e `InvalidChunkLength` só existe em resposta chunked, então a sonda era cega ao mecanismo por construção. **Antes de rodar: calcule a potência E confira que o alvo exercita o caminho** |
| 22/09 06:4x | **Eu derrubei a sessão do soak duas vezes, sem querer** | O teste de contrapressão logou como `admin`, e um login novo da MESMA conta invalida o token anterior (`WebManager_Auth.cpp:462`). O soak registrou `session renewed (Unauthorized)` e seguiu. Vale a regra: **teste que roda ao lado de uma bateria longa usa conta própria** |
...
| 21/09 11:52 | T3: **uma resposta do `/api/status` truncada** (`InvalidChunkLength`) em 61 amostras (5,1 h) | Sem reboot (uptime seguiu), e a amostra seguinte leu normal. **Nenhum dos quatro contadores de aborto do aparelho mexeu**: `ad`=0, `cgd`=0, `cgg`=0, `cgx` parado em 5. Causa não estabelecida — vira taxa medida depois do soak, com um laço fechado de requisições. Atenção: os 5 `cgx` são **meus**: subiram às 10:52, quando o laço do dreno caiu e os downloads quebraram; não são comportamento do aparelho |
...
**O passo `admin` do `panel_users_hw_test.py` não foi rodado.** Ele executa
...
passaram — a segunda linha está sã. Atenção: o `alarm show` também **mostra** o
...
**`SYS_BOOT` só é gravado quando houve algo a dizer** (`LogManager.cpp`
