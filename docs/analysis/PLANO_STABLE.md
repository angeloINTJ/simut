...
| | |
|---|---|
| **B1** | aberto — é o último, e o achado sobre `ctx=227` muda como se lê o resultado do T2 (ver nota da autópsia). **E ele é mais estreito do que parecia**: o reprodutor usa `write memory`, que a imagem publicada não tem, e o gatilho equivalente dela — commit web gravando flash com o display vivo — deu **0 em 500**. O T2 ainda precisa rodar na `pico_w_test`, onde o defeito é alcançável, porque o mecanismo (flash × Core 1) é o mesmo nas duas |
