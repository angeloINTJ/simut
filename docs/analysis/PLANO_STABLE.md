...
| 22/09 06:4x | ⚠️ **Eu derrubei a sessão do soak duas vezes, sem querer** | O teste de contrapressão logou como `admin`, e um login novo da MESMA conta invalida o token anterior (`WebManager_Auth.cpp:462`). O soak registrou `session renewed (Unauthorized)` e seguiu. Vale a regra: **teste que roda ao lado de uma bateria longa usa conta própria** |
