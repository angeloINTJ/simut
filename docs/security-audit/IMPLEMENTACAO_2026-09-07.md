...
| V-05 | `ap` → console mostra a PSK → notebook entra; `nmcli dev wifi list` mostra WPA2 | ⚠️ **METADE, 09/09 (v2.4.1-beta)**: `ap` subiu `simuttft_SETUP`, imprimiu uma PSK de **10 caracteres rotulada `(WPA2)`**, nenhuma linha de rede aberta, e `reload confirm` devolveu o aparelho à STA com a web de volta. **A metade pelo ar — um cliente Wi-Fi ver WPA2 na varredura e entrar com a PSK — não foi feita: o host da bancada não tem adaptador Wi-Fi** (`nmcli dev`: só ethernet e tailscale). Fica para um celular/notebook |
