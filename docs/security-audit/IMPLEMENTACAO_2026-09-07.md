...
| V-04 / O-2 | `tools/json_escape_cases.py` | ✅ **A/B RODADO 09/09** — release v2.4.1-beta (uf2 `59780203`) × pré-fix `8d043ed` (worktree, flash pela mão). **Caso 1 (SSID `a"b`):** as duas imagens ACEITAM a escrita (aspas é byte legal de SSID); na pré-fix o `/api/network` **não parseia**, na release **parseia** com `ssid='a"b'` — o escape é o que o fix entrega, e o A/B discrimina. Observado pelo console (`system ssid` salva sem reconectar), porque gravar pela web reinicia o aparelho numa rede que não existe. **Caso 2 (hwId `X\\`):** pré-fix ACEITA (200); release RECUSA com `400 {"error":"slot 0: invalid hwId"}` (O-2, `isValidHwId`) — discrimina na escrita. ⚠️ A metade "a resposta quebra" do caso 2 NÃO reproduziu na pré-fix por este caminho (o `/api/status` continuou parseando). **Caso 3 (pack `@NAME`): não rodado.** ⚠️ A ferramenta precisou de **dois** consertos para medir alguma coisa — mandava JSON cru (o firmware lê `_payload`) e a seção `sensors` (o firmware lê `slots`); nas duas formas o 400 vinha antes de olhar a string e ela dava PASS em todas as imagens. Só o A/B viu |
| V-05 | `ap` → console mostra a PSK → notebook entra; `nmcli dev wifi list` mostra WPA2 | ✅ **RODADO INTEIRO 09/09 (v2.4.1-beta)**, com adaptador Wi-Fi USB no host: `ap` sobe `simuttft_SETUP` e imprime a PSK; a varredura do host mostra **`WPA1 WPA2`** (não é rede aberta — é o mixed mode default do `softAP`); o host entrou com a PSK do console (IP 192.168.4.16/24); `GET http://192.168.4.1/` → 302 `/login`, `/api/login_init` → 200; `reload confirm` devolveu à STA. ⚠️ **Resíduo, baixo:** o AP anuncia WPA1/TKIP junto com WPA2 (`CYW43_AUTH_WPA2_MIXED_PSK`); WPA2-only seria `CYW43_AUTH_WPA2_AES_PSK` — não bloqueia o achado, fica anotado |
