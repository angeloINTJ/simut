...
|   · F-CSV.1 — helper `crc32_init/update/final` em `SystemUtils.cpp` (CRC32-IEEE bitwise, mesma matemática de `StorageManager::calculateCRC32` mas exposto incremental para streaming). Forward decls em `SystemDefs_Records.h`. | 🟡 Implementado, HW pendente | — | — | 2026-05-01 |
|   · F-CSV.2 — `GET /api/export/history.bin?from=&to=&sensors=` (cap 31d, formato `.simx` kind='H' + CRC32 trailer). | ⚪ Pendente | — | — | — |
|   · F-CSV.3 — `GET /api/export/logs.bin?from=&to=&level=err\|inf\|all` (formato `.simx` kind='L' + CRC32 trailer). | ⚪ Pendente | — | — | — |
|   · F-CSV.4 — UI `/hist` abaixo do gráfico: 2 pickers date+time + dropdown sensor + botão. JS divide range em meses calendar, fetch sequencial, valida CRC32, decodifica `.simx`, gera CSV (UTF-8 BOM, ISO-8601), trigger download `simut_history_<YYYY-MM>.csv`. | ⚪ Pendente | — | — | — |
|   · F-CSV.5 — UI `/hist` abaixo dos logs: 2 pickers + dropdown nível + botão. Reusa decoder `.simx` para logs, lookup `code`→texto via `/api/lang`. Download `simut_logs_<level>_<YYYY-MM>.csv`. | ⚪ Pendente | — | — | — |
|   · F-CSV.6 — i18n PT/ES/EN + `STABILITY_PLAN.md` (esta entrada) + `SECURITY.md` (auth + rate-limit dos novos endpoints) + release `v3.28.0`. | ⚪ Pendente | — | — | — |
