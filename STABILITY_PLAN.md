...
| U24 | 🔴 | F11 | ✅ | **Commit-all + reboot pattern (2026-04-19):** rajadas de saves consecutivos eram a fonte original de todos os bugs de concorrência (U16/U21/U23). Mudança arquitetural do modelo UX: interface web acumula mudanças no `sessionStorage` client-side; botão único "Salvar e Reiniciar" no topbar (só aparece se há pendentes). Ao clicar: confirmação com aviso de risco → POST `/api/commit_all` com JSON → server aplica tudo em 1 save → reboot limpo. Phase A.1 (este commit): migrado `/config` ao novo fluxo; botões individuais Save removidos; audit log bufferizado via `lockHeavyTask()` antes do save pra drenar em 1 flash write; display mostra "Aplicando configuracoes..." / "Reiniciando o sistema..." (3s total) antes do flash I/O; pós-save usa `watchdog_enable(500, 1) + while(1)` pra reboot garantido em 500ms mesmo se algo travar. Endpoints legacy (`/api/save_sys`) mantidos por compat mas UI não chama. Phase A.2: migrar `/alarms`. |
