...
| **F-OTA (trilha OTA self-flash)** | ✅ FECHADA (loop20 100% PASS em v3.45.0, 2026-05-09) | `feature/ota-self-flash` | `v3.38.0..v3.45.0` | 2026-05-01..2026-05-09 |
...
|   · **F-OTA-CFG-PRESERVE (Fase 9)** — snapshot de `/config/system.bin` preservado através do apply destrutivo. Iterações: v3.43.14 (snapshot na metadata partition pages 1..15, `s_applier_buf` extern) — bricked boot; v3.43.15 (split serialize/commit) — mesmo bug; v3.43.16 (snapshot no último setor da staging area, offset `OTA_SNAPSHOT_OFFSET = 0x1FE000`) — **DESIGN VALIDADO** em HW 2026-05-07: chpass + 11/11 campos `/api/config` preservados byte-a-byte após apply destrutivo. Hook pré-stage em `staging_session_begin` (serialize lê LFS sem lockout, commit grava após `staging_erase_all`). Hook pós-apply em `StorageManager::begin` (`ota_snapshot_restore_to_lfs` antes de `loadConfiguration`). Header `ConfigSnapshotHeader` 16 B + payload + CRC32 EDB88320. Demais arquivos (history/lang/themes/calib) via `.bkp` baixado pelo navegador antes do upload (fallback A). | ✅ DESIGN VALIDADO em HW (v3.43.16) | — | `v3.43.16` | 2026-05-07 |
|   · Custo: +720 B flash (98.6%), zero RAM nova. Sketch máximo seguro mantendo snapshot intacto = 1.016 KiB (atual ~1.004 KiB → margem 12 KiB). | — | — | — | — |
|   · **🔴 BUG RESIDUAL F-OTA-BOOTLOOP**: boot pós-apply trava antes de subir Web/CLI mesmo após snapshot restore funcionar. Apenas **power cycle físico** (USB unplug+replug) destrava — reset por GPIO RUN (mão pico_hand HOLD RESET 10s) não basta. Sintoma similar ao Bug 2 do `v3.43.11` (PSM full reset) que foi parcialmente corrigido. Hipótese: estado de hardware periférico (CYW43, SIO mailbox, USB CDC, BTstack TLV?) não é limpo por `watchdog_reboot` mesmo com `PSM->wdsel = 0xFFFFFFFF`. Investigação aberta como F-OTA-BOOTLOOP. | 🔴 Bloqueia uso real do OTA | — | — | 2026-05-07 |
...
| **F-OTA-BOOTLOOP** — boot pós-apply travado até power cycle físico | ✅ FECHADO (v3.45.0, loop20 20/20 PASS, 2026-05-09) | `feature/ota-self-flash` | `v3.45.0` | 2026-05-07..2026-05-09 |
|   · Sintoma: após `ota_apply_pending_update` rodar (HTTP 202 OK + applier copia staging→app slot + `applier_reboot` via watchdog/PSM), o SIMUT enumera USB CDC mas Web (porta 80) e CLI (Serial USB) ficam mudos indefinidamente. Reset por GPIO RUN não recupera; apenas power cycle físico do USB sobe o sistema. Após power cycle, LFS já tem `system.bin` restaurado (snapshot funcionou) → boot completa normal. | — | — | — | — |
|   · Reproduzido em: v3.43.14, v3.43.15, v3.43.16 (todas as variantes da Fase 9). v3.43.10 e v3.43.11 (sem Fase 9) reportaram apply OK em HW — possível que o problema seja específico do path com snapshot restore, ou mais provável que é o mesmo Bug 2 que já existia mas com sintoma agravado. | — | — | — | — |
|   · Próximos passos investigação: instrumentar Serial logging detalhado em `AppManager::setup`, comparar boot path normal (USB flash) vs pós-apply (watchdog reboot), revisar tear-down do orchestrator (CYW43, LittleFS, IRQs), pesquisar RP2040 reset peripherals docs. | — | — | — | — |
|   · **F-OTA-BOOTLOOP fix #3 (v3.43.21, 2026-05-07)**: causa raiz parcial encontrada — `applier_reboot` setava `ENABLE\|TRIGGER` no watchdog. Watchdog HW NÃO é resetado por PSM (não está em `PSM_WDSEL_BITS`), só por power cycle físico. Pós-reset, ENABLE persistia + LOAD curto = boot loop infinito até power cycle. Fix: TRIGGER apenas (igual SDK pico-sdk com `delay_ms=0`); `LOAD = 0xFFFFFF` (24-bit max ≈ 8s). Resultado: brick rate caiu de 100% (toda iter) para **~24% residual** (4 brick em 17 tentativas válidas no `loop20`). PASS rate: **76%** vs antes 0%. Bricks remanescentes seguem padrão "4-7 PASS consecutivos seguidos de 1 brick" — sugere acúmulo de estado entre apply consecutivos (snapshot offset / LFS / CYW43). Investigação continua. | 🟡 Parcialmente resolvido | — | `v3.43.21` | 2026-05-07 |
|   · **Alpha v3.44.0-alpha1 (2026-05-07)**: UI safety pré-OTA — modal de aviso obrigatório, backup auto-download com verificação de integridade via headers HTTP `X-Backup-PSize`/`X-Backup-PCrc` (browser parseia header BKP1 do .bkp baixado e compara com headers anunciados; abort se mismatch). Validação server-side de firmware mantida (`/api/restore?op=stage` retorna `v=6` se boot2 CRC fail). Reservado: **v4.0.0** será o release que resolver bricks residuais. Loop20 em v3.44.0-alpha1 pendente para confirmar que UI safety não regrediu pass rate. | 🟡 Em andamento | — | `v3.44.0-alpha1` | 2026-05-07 |
|   · **Alpha v3.44.0-alpha9 (2026-05-08)**: Fix Achado #5 v2 CORRETO em `LogManager::safeReboot` — replica EXATAMENTE a sequência MMIO de `applier_reboot` Fix #3 v3.43.21 (PSM_WDSEL → CLR_ENABLE → scratch[4]=0 → LOAD=0xFFFFFF → TRIGGER). Alpha4 tentou mas omitiu CLR_ENABLE + scratch[4]=0 com ordem trocada → regressão grave. Alpha5-8 reverteram para `watchdog_enable(500,1)` original (instável: brick ocasional em `reload confirm`). Validação alpha9 HW 2026-05-08: ✅ boot picotool, ✅ `reload confirm` recovery em 60s (era 100% brick em alpha4), ❌ OTA apply ainda brick (residual em outro path — `applier_reboot` já é Fix #3 desde v3.43.21). Outras melhorias: `tools/test_f9_snapshot.sh` timeouts enxutos (240s→80s no ota_apply, removida cycle 2), `tools/ota_apply.py` wait_for_device 180s→30s (wrapper faz wait extra), `tools/PicoHand` v3 com `INPUT_PULLUP` em pin_release (empírico em HW: `pinMode(INPUT)` puro às vezes não libera linha), `tools/test_firmwares/pico_blink_echo` (firmware mínimo de revival pra brick recovery — user 2026-05-08: "Pico volta à vida com firmware bem básico"), `tools/test_f9_loop20.sh` recovery atualizada com blink revival pattern. Tentativas falhas: alpha6 (wrap snapshot_restore com dois pares enter/exit consecutivos = boot brick), alpha7 (single enter/exit pair = deadlock reentrante porque LFS internamente já usa multicore_lockout via flash_safe_execute). | 🟡 Em andamento | — | `v3.44.0-alpha9` | 2026-05-08 |
|   · **Alpha v3.44.0-alpha20 (2026-05-09)**: Fix backup restore reboot mid-transfer — `handleApiRestoreUploadData` agora envolve `restore_session_feed` em `RenderGuard` quando `mode==APPLY`. Sem isso, Core 1 (SPI ILI9341) corria paralelo a `flash_program_page` no Core 0 durante `cur_file.write` da LFS → lockout deadlock → WDT 15s → reboot mid-upload do `.bkp`. Mesmo padrão usado em `WebManager_Files::_flushUploadBatch` (que funcionou em prod). VALIDATE mode (não escreve LFS) NÃO usa pause — `rg(nullptr)`. Reportado pelo user 2026-05-09: "A restauração do backup está provocando a reinicialização do sistema antes de concluir a transferência." **VALIDADO em HW 2026-05-09** com FS 85.5% cheia (897 KB de 1024 KB usados, 32 files, backup 770.6 KB): 5/5 APPLY OK com backup 7.4 KB + **10/10 APPLY OK consecutivos com backup 770.6 KB** (27s/restore, ~28 KB/s); device sem reboot durante toda a sequência (uptime preservado), heap estável (58.4 KB free), FS íntegra pós-stress, web e CLI responsivos. | ✅ HW VALIDADO | — | `v3.44.0-alpha20` | 2026-05-09 |
|   · **Alpha v3.44.0-alpha22 → REVERTIDO em alpha23 (2026-05-09)**: Fix #6 candidato — power-cycle CYW43 chip via `WL_REG_ON` (GPIO 23) NO INÍCIO de `AppManager::setup`. Hipótese: drive LOW 100ms → HIGH 50ms ANTES do SDK pegar ownership = chip fresh sem mexer no path do reboot. **Falhou em HW 2026-05-09**: alpha22 brickou em 1/1 iter de teste OTA (F-USB-CDC-DEAD imediato pós-apply). Manipular WL_REG_ON antes do `cyw43_arch_init` confunde o SDK (spec do SDK assume estado conhecido inicial do pin). Reversão em alpha23. Conclusão: F-OTA-BOOTLOOP residual ~24% NÃO é resolvido por manipulação de WL_REG_ON em soft path — única recovery confirmada continua sendo power cycle físico (USB unplug+replug). Próxima investigação requer instrumentação UART (não USB CDC, que vai mute na hora do brick) pra ver onde exatamente o boot pós-apply trava. Histórico do dia: alpha21 iter 1 PASS, iter 2 FAIL (via test_f9_snapshot); alpha22 iter 1 FAIL; reversão alpha23 também não bota sem power cycle físico (chip destruído pelos testes anteriores). | 🔴 Sessão sem fix viável | — | `v3.44.0-alpha22→23` | 2026-05-09 |
|   · **Sessão 2026-05-09 — alpha22→alpha31 (8 commits)**: trabalho intensivo de debug do F-OTA-BOOTLOOP residual. **Wins major**: (a) alpha20 backup restore mid-transfer fix (HW validado 10/10), (b) alpha24-25 **UART debug bridge GP8/GP9** entre PicoHand e SIMUT — ferramenta diagnóstica que destravou todo o debug subsequente, (c) alpha27 fix do README.md write reentrante em StorageManager::begin (mesmo bug do alpha8 que havia sido re-introduzido em alpha21), (d) alpha28 defer SerialBT pra pós-WiFi.begin, (e) alpha30 scratch[5]=POST_OTA_APPLY_MAGIC infra + WL_REG_ON cycle (mecanismo OK, não era root cause), (f) **alpha31 fix #9 — REMOVIDO enterFlashSafeMode wrap dos mkdirs** + applier ERASE staging/LFS region antes do reboot. Boot post-flash + post-RESET completa todos 14 steps via UART. **Pendente**: boot post-safeReboot/post-OTA ainda hang PRE-UART-init (não consegui localizar via UART porque UART não foi inicializada ainda). Próxima sessão precisa diagnóstico via scratch register persistente OU UART alternativa via PicoHand UART. Toda infra (UART bridge, scratch magic, markers) está estabelecida no GitHub. | 🟡 Major progress, OTA pending | — | `v3.44.0-alpha31` | 2026-05-09 |
|   · **Memórias e workflow consolidados (2026-05-08)**: nova autonomia full pra commit/push/iterate até v4 estável (`feedback_v4_autonomy.md`); regra timeout 3 min cap (`feedback_test_timeout_3min.md`); técnica blink revival pra brick recovery (`feedback_pico_revival.md`); preferir firmwares de teste isolados sobre power cycle (`feedback_test_firmwares_individual.md`). Documentação acumulada em `docs/INVESTIGATION_BOOTLOOP.md` com Achado #5 resolvido e Achado #6 (snapshot race) descartado (LFS já protege via flash_safe_execute). | — | — | — | — |
|   · **🎯 v3.44.0-alpha34/alpha35 — F-OTA-BOOTLOOP RESOLVIDO (2026-05-09)**: alpha34 fix #10 — `scanCustomThemes()` removeu `LittleFS.mkdir(THM_DIR)` do boot (mkdir só roda em `handleApiUpload`, path seguro com Core 1 ativo). alpha35 fix #11 — `_displayMgr->startCore1()` deferido de step 3 para step 6.5 (pós `StorageManager::begin`). Root cause final: `multicore_lockout` IRQ-based usado por `flash_safe_execute` do LFS frequentemente trava em boot pós-OTA porque Core 1 entra em estado onde IRQ inter-core não responde. Com Core 1 INACTIVE durante mountFS+mkdirs+snapshot restore+loadConfig, `flash_safe_execute` usa single-core path (só `disable_interrupts` local) — sem multicore lockout = sem hang. Trade-off aceito: TFT sem boot status até step 6.5 (~7s extra).<br><br>**Validação HW 2026-05-09**: `tools/test_f9_loop20.sh` com break-on-fail → **20/20 PASS, 0 FAIL, 0 recoveries necessários** (`docs/test_reports/f9_loop20_20260509-134813.log`). Tempo médio: 75s/iter. Boot post-OTA captured via UART markers: `01346789ABE` (StorageManager mountFS+mkdirs+snapshot+loadConfig todos OK) → `CR` (Core 1 deferido + ready) → step 14 SYS READY @ 18.5s. Brick rate: alpha31 ~24% → alpha35 **0%**. Tag interna `v3.45.0` marca fechamento da fase. **Release público v4.0.0 ainda bloqueado por F-OTA-RAM** (uzlib 33 KiB BSS, ver blocker abaixo). | ✅ FECHADO | — | `v3.45.0` | 2026-05-09 |
| **F-PICOHAND** — mão robótica para automação OTA | ✅ HW validada | `feature/ota-self-flash` | — | 2026-05-06 |
|   · `tools/PicoHand/`: outro Pico rodando firmware dedicado aciona BOOTSEL (GP0) + RUN (GP1) do alvo via fiação direta. Open-drain emulado, comandos serial USB CDC (`PING`, `BOOTSEL`, `RESET`, `HOLD/RELEASE`, `STATUS`, `PINOUT`, `SELF_BOOTSEL`). Wrapper bash + manual escrito explicitamente para o agente. Destrava ciclos automatizados de OTA — sem ela cada brick exigia reset físico humano. **Nota: wrapper `pico_hand.sh` tem bug em USB CDC com write+read em redirects separados; em automação use Python `serial.Serial` (FD persistente).** | — | — | — | — |
