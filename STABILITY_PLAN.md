...
| U24 | 🔴 | F11 | ✅ | **Commit-all + reboot pattern (2026-04-19):** rajadas de saves consecutivos eram a fonte original de todos os bugs de concorrência (U16/U21/U23). Mudança arquitetural do modelo UX: interface web acumula mudanças no `sessionStorage` client-side; botão único "Salvar e Reiniciar" no topbar (só aparece se há pendentes). Ao clicar: confirmação com aviso de risco → POST `/api/commit_all` com JSON → server aplica tudo em 1 save → reboot limpo. **Phase A.1** (v3.15.0): migrado `/config`; audit log bufferizado via `lockHeavyTask()` antes do save pra drenar em 1 flash write; display mostra "Aplicando configuracoes..." / "Reiniciando o sistema..." (3s total); pós-save usa `watchdog_enable(500, 1) + while(1)` pra reboot garantido em 500ms. **Phase A.2** (v3.16.0): migrado `/alarms` (sensors + sounds); handlers legacy `handleApiSaveAlarms` e `handleSaveSystem` (versão grande) removidos — `handleSaveSystem` reduzido a versão minimal só pra theme-switch do dashboard. **Phase B** (v3.17.0): migrado `/users` como queue de ações (`add`/`del`/`reset`); UI mostra overlay visual dos pendentes (linha nova verde com badge, strikethrough, badge "Pendente: Reset"); botão ↶ pra desfazer último pending add; server aplica ações em ordem; legacy `handleApiUserAdd/Del/Reset` removidos. Próxima: Phase C (`/network`). |
